...
- Notes: The loader writes directly into curated ClickHouse tables created by
  ...
  and exposes `clear-failed` for deliberate recovery after a failed or
  interrupted append-once load. Loading remains fail-fast: a failed source
  stops the selected batch; cleanup later removes only sources recorded as
  started or failed. Validation reports cover row counts, date ranges, key
